...
CHROMIUM_VER=148.0.7778.56
CHROMIUM_NODE_VER=24.12.0 # What version of nodejs does Chromium need to build?
CHROMIUM_PATCH_VER="${CHROMIUM_VER%%\.*}-3"
CHROMIUM_TARBALL_URI="https://github.com/chromium-linux-tarballs/chromium-tarballs/releases/download/${CHROMIUM_VER}"
GN_MIN_VER=0.2354 # Must match GN version used by base Chromium release (no compat guarantees)
NODE_VER=24.15.0 # The node we're building for electron, not the one we use to build it.
...
LICENSE+=" SGI-B-2.0 SSLeay SunSoft Unicode-3.0 Unicode-DFS-2015 Unlicense UoI-NCSA ZLIB libtiff openssl"
LICENSE+=" rar? ( unRAR )"
...
src_prepare() {
	...
	local vector_sed="s/std::vector<const std::vector</std::vector</g"
	local node_dir_sed="s|NODE_DIR = os.path.join|NODE_DIR = os.path.abspath(os.path.join|g;"
	node_dir_sed+=" s|'electron_node')|'electron_node'))|g"

	declare -A sed_files=(
		# PF4Public/gentoo-overlay
		["patches/chromium/feat_add_data_parameter_to_processsingleton.patch"]="${vector_sed}"
		["shell/browser/api/electron_api_app.cc"]="${vector_sed}"
		["shell/browser/api/electron_api_app.h"]="${vector_sed}"
		["shell/common/keyboard_util.cc"]="s/constexpr CodeAndShiftedChar/CodeAndShiftedChar/g"
		["script/generate-config-gypi.py"]="${node_dir_sed}"
	...
	grep "'--openssl-no-asm'" "${WORKDIR}/${P}/script/generate-config-gypi.py" > /dev/null ||
		die "Failed to find '--openssl-no-asm' in script/generate-config-gypi.py"
	NODE_CONFIG_ARGS="'--without-bundled-v8', '--shared-openssl', '--shared-zlib'"
	NODE_CONFIG_ARGS+=", '--without-npm', '--shared-cares', '--shared-http-parser', '--shared-nghttp2'"
	...
	local patch
	for patch in "${WORKDIR}/chromium-patches-${CHROMIUM_PATCH_VER}"/**/*.patch; do

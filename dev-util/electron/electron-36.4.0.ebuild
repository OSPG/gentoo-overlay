...
CHROMIUM_PATCH_VER="${CHROMIUM_VER%%\.*}-2"
...
NODE_VER=22.15.1 # The node we're building for electron, not the one we use to build it.
...
src_prepare() {
	...
	pushd "${WORKDIR}/${P}" > /dev/null || die "Failed to change directory to ${WORKDIR}/${P}"
	eapply "${FILESDIR}/${P}-stl-iterator.patch"
	popd > /dev/null || die "Failed to change directory back to ${WORKDIR}"
